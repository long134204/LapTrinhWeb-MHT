...
public class BookRepositoryImpl implements IBookRepository {
    ...
    public List<Book> search(String inputSearch, int page, int size) {
        ...
        try {
            Connection connection = BaseRepo.getConnection();
            PreparedStatement preparedStatement = connection.prepareStatement("select * from book where available >0 and (book.isbn like ? or book.title like ?) and `status` =? LIMIT ?, ?" );
    ...
    public Boolean checkIsbn(String isbn) {
        ...
        try {
            Connection connection = BaseRepo.getConnection();
            PreparedStatement preparedStatement = connection.prepareStatement("SELECT count(isbn) as total FROM management_system_library.book where isbn = ? and `status` = ?");
            preparedStatement.setString(1, isbn);
